...
def add_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    # 新设定：treatment/action 个数（原来的 n_classes 其实就是这个）
    parser.add_argument("--n_actions", type=int, default=10,
                        help="number of categorical treatments/actions A in {0,...,K-1}")

    parser.add_argument("--seed_train", type=int, default=0)
    parser.add_argument("--seed_test", type=int, default=42)

    # 新设定：是否用非线性 data 生成 (phi(X))，以及 y 的噪声强度
    parser.add_argument("--nonlinear", action="store_true",
                        help="use nonlinear feature map in synthetic data generation")
    parser.add_argument("--y_noise", type=float, default=1.0,
                        help="std of noise added to continuous outcome y")

    # （可选）如果你想保留“分类/回归”两种任务的扩展口
    parser.add_argument("--task", type=str, default="reg", choices=["reg", "cls"],
                        help="reg: continuous y regression; cls: classification (old version)")
    ...
    parser.add_argument("--mu_tether", type=float, default=1.0,
                        help="0.5*mu*||beta - Z||^2 during finetune")
    ...
    parser.add_argument("--round_decimals", type=int, default=2,
                        help="round decimals used for clustering/dendrogram")
