...
def run_admm_path(
    ...
    Returns:
        all_Z: list[np.ndarray]  each element is Z(lam) with shape (C, d)
        metrics: list[dict]
        Z_final: torch.Tensor
        U_final: torch.Tensor

    NEW: if outdir is provided, saves:
        - Z_path_{save_tag}.npy  (shape: [L, C, d])
    ...
    for li, lam in enumerate(lambda_values):
        ...
        for _ in range(int(args.admm_epochs)):
            # ---- theta/beta update: approx minimize CE + (rho/2)||beta - Z + U||^2
            for _ in range(int(args.theta_steps)):
                opt_beta.zero_grad()
                beta = model.output_layer.weight
                logits = H_train @ beta.T
    ...
    if outdir is not None:
        ...
        if len(all_Z) > 0:
            Z_path = np.stack(all_Z, axis=0)  # (L, C, d)
