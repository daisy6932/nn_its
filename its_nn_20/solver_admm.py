...
def run_admm_path(
    ...
    for li, lam in enumerate(lambda_values):
        ...
        with torch.no_grad():
            if hasattr(model, "predict_from_HA"):
                y_hat_end = model.predict_from_HA(H_train, A_train).view(-1)
                y_train_flat = y_train.view(-1)
                reg_end = float(_reg_loss(y_hat_end, y_train_flat, loss_type, huber_delta).item())
                train_mse = float(F.mse_loss(y_hat_end, y_train_flat).item())
            else:
                reg_end = float("nan")
                train_mse = float("nan")
        msg = (
        ...
        val_mse = None
        if H_val is not None and A_val is not None and y_val is not None:
            with torch.no_grad():
                y_hat_val = model.predict_from_HA(H_val, A_val).view(-1)
                y_val_flat = y_val.view(-1)
                val_mse = float(F.mse_loss(y_hat_val, y_val_flat).item())
