...
def add_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    # ----- lambda / cycle selection -----
    parser.add_argument("--lambda_pick", type=str, default="val_tol_simplest_stable",
                        choices=[
                            "last", "middle", "reg", "reg_plus_c3",
                            "val", "val_plus_c3",
                            "val_tol_simplest", "val_tol_simplest_stable"
                        ])

    parser.add_argument("--lambda_complexity_weight", type=float, default=0.01)

    parser.add_argument("--cycle_metric", type=str, default="val_plus_stability",
                        choices=["val", "val_plus_stability"],
                        help="criterion used to compare cycles")

    parser.add_argument("--cycle_improve_tol", type=float, default=1e-4,
                        help="minimum validation improvement to count as progress")

    parser.add_argument("--cycle_patience", type=int, default=2,
                        help="stop if no cycle-level improvement for this many cycles")

    parser.add_argument("--stability_metric", type=str, default="fro",
                        choices=["fro", "labels"],
                        help="how to measure similarity between consecutive cycles")

    parser.add_argument("--stability_tol", type=float, default=1e-3,
                        help="threshold for declaring clustering stable")

    parser.add_argument("--stability_weight", type=float, default=0.0,
                        help="penalty weight on instability when comparing cycles")

    parser.add_argument("--save_cycle_summary", action="store_true",
                        help="save cycle-level selection summary to disk")

    parser.add_argument("--lambda_val_tol_abs", type=float, default=None)
    parser.add_argument("--lambda_val_tol_ratio", type=float, default=0.01)

    parser.add_argument("--train_improve_tol", type=float, default=1e-6)
    ...
    parser.add_argument(
        ...
        choices=["full", "backbone_only", "backbone_plus_baseline"]
    )
    return parser